  ...
  <div class="app-container">
    ...
    <main ref="inputCardRef" class="glass-card input-group">
      ...
      <div class="options-row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
        <div class="selector-group">
          <label class="selector-label">实习岗位</label>
          <div class="style-selector">
            <button 
        ...
        <div class="selector-group">
          <label class="selector-label">汇报类型</label>
          <div class="style-selector">
            <button 
      ...
      <div class="selector-group">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          ...
          <div style="display: flex; gap: 0.5rem;">
            ...
            <button class="text-link-btn" @click="showGuideModal = true">实习报告撰写与转正答辩通关指南</button>
          </div>
        </div>
        <textarea 
          v-model="userInput" 
...
const appTitle = ref(appConfig.title || '网腾无限AI - 大学生实习报告与转正总结专家');
...
const presetOptions = [
  "互联网大厂产品经理 3 个月实习总结报告",
  "前端/后端开发实习生转正答辩 PPT 大纲",
  "新媒体运营实习生月度数据与成果汇报",
  "高校要求的标准 3000 字毕业实习鉴定表评语"
];
...
const metricsList = [
  {
    "key": "internshipTaskClarity",
    "label": "实习工作任务清晰度"
  },
  {
    "key": "achievementQuantification",
    "label": "成果数据量化度"
  },
  {
    "key": "reflectionGrowthDepth",
    "label": "反思成长总结深度"
  },
  {
    "key": "professionalCompetencyFit",
    "label": "职业胜任力契合分"
  },
  {
...
const guideRules = [
  {
    "title": "成果量化拒绝流水账",
    "advice": "用“独立完成 X 篇推文，阅读量提升 Y%，转化注册 Z 人”代替“完成了文章撰写”",
    "avoid": "切忌平铺直叙列举日常杂事"
  },
  {
    "title": "主观能动性体现",
    "advice": "突出自己在面对困难时的主动思考与解决方案，展现快速学习与抗压能力",
    "avoid": "避免显得自己只是一个被动接收指令的机械执行者"
  },
  {
    "title": "转正规划接地气",
    "advice": "转正规划要切中部门下阶段的实际业务痛点，明确自己的可产出价值",
    "avoid": "严禁提出高高在上缺乏可行性的空洞大口号"
  }
];
...
const displayResultText = computed(() => {
  ...
  return result.value.replace(/\[SHIXI_SCORES\][\s\S]*?\[\/SHIXI_SCORES\]/g, '').trim();
});

const parseAiScores = (rawText: string) => {
  const match = rawText.match(/\[SHIXI_SCORES\](.*?)\[\/SHIXI_SCORES\]/);
...
const loadHistory = () => {
  try {
    const raw = localStorage.getItem('shixi_history_records');
...
const saveHistory = () => {
  localStorage.setItem('shixi_history_records', JSON.stringify(historyList.value));
};
